...
export function activateStandardMotionExtension(
  ...
  const disposeRegisteredContributions = (): void => {
    ...
    if (failures.length > 0) {
      throw new AggregateError(
        failures,
        "Failed to dispose all Standard Motion contributions"
      );
    }
  };
  ...
  try {
    ...
    disposers.push(context.transactions.registerDocumentValidator({
    ...
    disposers.push(
      context.evaluation.registerDocumentEvaluator(standardMotionDocumentEvaluator)
    );
  } catch (caught) {
    try {
