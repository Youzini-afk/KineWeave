...
- `project-session` 只组合内存领域服务；Node Repository 的连续保存和文件事务由 `project-session-node` 适配。
- Distribution Profile 提供发行版已安装扩展和 Capability 默认选择，不取代工程需求或 Lockfile。
...
当前 TypeScript 扩展使用真实的 `in-process` Runtime 标识。ESM Loader 在 Resolve 之后才导入所选 Entrypoint，并按 Manifest 的 `exportName` 获取激活函数。激活完成后，ProjectSession 核对 Manifest 声明的 Document Type、Operation、Evaluator 和 Capability Provider 与仍处于注册状态的贡献；不一致时拒绝打开工程并回收本轮激活结果。
...
两条渲染链路与 Studio 真实交互用于持续施压边界，不表示 Presentation Graph 或 Standard Motion 已经封口。下一步以 Golden Project、跨 Renderer Conformance、长帧序列和桌面端到端场景继续检验语义一致性、求值缓存与性能边界。
...
当前进度：工程状态、精确时间求值、Presentation Graph、Capability 选择、SVG 文件输出和 Canvas2D 交互预览已经贯通；同一图语义已经经过文件输出与真实桌面 Stage 两类 Renderer 验证，CLI 与 Studio 共享求值语义。
...
当前进度：Workbench Shell、Stage、播放头、图层、Inspector、历史编辑、Headless Render CLI 已贯通；Golden、Conformance、桌面 E2E、性能预算与 CI 为本 Foundation 的剩余质量工作。
